...
void setup()
{
  ...
  memory_begin = (uint32_t*)malloc(extmem_size << 20);
  uint8_t size = (memory_begin == NULL) ? 0 : extmem_size;
  psram_size = size * 1024 * 1024;
  memory_end = memory_begin + (psram_size / sizeof(uint32_t));
  psram_sizeMB = (float)size;

  while (!Serial) ; // wait
  Serial.printf("\n\nEXTMEM Memory Test, %d Mbyte @ %p\n", size, memory_begin);
  if (size == 0) return;

  Serial.printf("EXTMEM frequency: %.2f MHz\n", (198e6)/*extmem_freq()*/ / 1e6f);
